...
SELECT
    sd.skills,
    ROUND(MEDIAN(jpf.salary_year_avg), 0) AS medium_salary,
    COUNT(jpf.salary_year_avg) AS demand_count,
    ROUND(LN(COUNT(jpf.*)), 1) AS ln_demand_account,
    ROUND((MEDIAN(jpf.salary_year_avg) * LN(COUNT(jpf.*)))/1_000_000, 2) AS opt_score,
...
┌────────────┬───────────────┬──────────────┬───────────────────┬───────────┐
│   skills   │ medium_salary │ demand_count │ ln_demand_account │ opt_score │
